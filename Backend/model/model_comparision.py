...
print("=" * 70)
print("70K LOAN DATASET - MODEL COMPARISON")
print("=" * 70)
...
print(f"CV F1:           {best_model['CV F1']:.4f}")
print(f"Train F1:        {best_model['Train F1']:.4f}")
print(f"Test F1:         {best_model['F1 Score']:.4f}")
...
plt.title(
    "Loan Approval Model Comparison - 70K Dataset",
    fontsize=15,
    pad=20
)
...
elements.append(
    Paragraph(
        "This report presents the performance comparison "
        "of four Machine Learning classification models "
        "using the 70K Loan Approval dataset.",
        styles["Normal"]
    )
)
...
dataset_data = [
    ["Information", "Value"],
    ["Training Samples", str(X_train_processed.shape[0])],
    ["Testing Samples", str(X_test_processed.shape[0])],
    ["Features", str(X_train_processed.shape[1])],
    ["Models Compared", "4"]
]
...
best_data = [
    ["Metric", "Score"],
    ["Model", best_model["Model"]],
    ["CV F1", f"{best_model['CV F1']:.4f}"],
    ["Train F1", f"{best_model['Train F1']:.4f}"],
    ["Accuracy", f"{best_model['Accuracy']:.4f}"],
    ["Precision", f"{best_model['Precision']:.4f}"],
    ["Recall", f"{best_model['Recall']:.4f}"],
    ["Test F1", f"{best_model['F1 Score']:.4f}"],
    ["Overfitting Gap", f"{best_model['Overfitting Gap']:.4f}"]
]
...
for index, row in results_df.iterrows():
    ...
    metrics_data = [
        ...
        ["Train F1", f"{row['Train F1']:.4f}"],
        ["Accuracy", f"{row['Accuracy']:.4f}"],
        ["Precision", f"{row['Precision']:.4f}"],
        ["Recall", f"{row['Recall']:.4f}"],
        ["Test F1", f"{row['F1 Score']:.4f}"],
        ["Overfitting Gap", f"{row['Overfitting Gap']:.4f}"]
    ]
